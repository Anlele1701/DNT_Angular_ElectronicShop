...
export class EditProductComponent implements OnInit{
  ...
  selectedImage: any[] = [];
  constructor(private activeRoute:ActivatedRoute, private router:Router, private http:HttpClient, private API: API){}
  getNameProduct(){
  ...
  getAllHang(){
    this.http.post(this.API.getAPI()+"/hang/getAllHangFromCate",{tenLoaiSP:this.loaiSP}).subscribe((data:any)=>{
  ...
  getProduct(){
    this.http.get(this.API.getAPI()+'/sanpham/getSP/'+this.idSP).subscribe((data:any)=>{
      this.product=data
  ...
  editProduct(){
    ...
    this.http.post(this.API.getAPI()+'/sanpham/editSanPham',pro).subscribe((data:any)=>{
      console.log(data)
